...
- The active game controls the existing type tools end-to-end: available type choices, the effectiveness engine, Study type lookup, Pokémon incoming/outgoing matchups, team matrices, and type-answer quizzes. One normalized type chart is cached for the exact selected version group. It is built from PokéAPI Type damage relations and historical damage relations, using the API’s “last generation” historical boundary, and is fetched before the app initializes or before a game change commits. A failed game-change fetch leaves the previous game and cache intact; changing games replaces the chart and clears other game-dependent lookup data.
