...
function isValidEvolution(value) { return value && Array.isArray(value.previous) && Array.isArray(value.next) && value.previous.every(name => typeof name === 'string' && name.length > 0) && value.next.every(name => typeof name === 'string' && name.length > 0); }
...
function findEvolutionContext(link, targetName, parentName = null) {
  ...
  if (link.species.name === targetName) return { previous: parentName ? [parentName] : [], next: (link.evolves_to ?? []).map(child => child?.species?.name).filter(name => typeof name === 'string' && name.length > 0) };
  for (const child of link.evolves_to ?? []) { const match = findEvolutionContext(child, targetName, link.species.name); if (match) return match; }
  return null;
}
