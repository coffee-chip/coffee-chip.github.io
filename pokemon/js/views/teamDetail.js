...
import { getTeam, removePokemonFromTeam, reorderPokemonInTeam, setTeamOpponent, setTeamPokemonAlias } from '../data/teamRepository.js';
...
function createTeamDetailTabs(render) {
  ...
  for (const [value, label] of [['members', 'Members'], ['matchups', 'Matchups'], ['advantage', 'Advantage']]) {
    ...
    button.addEventListener('click', () => {
      if (activeTeamDetailMode !== value) {
        activeTeamDetailMode = value;
        render();
      }
    });
    tabs.append(button);
...
function createMemberEditForm(team, member, pokemon, canonicalDisplayName, card, render) {
  ...
  input.type = 'text';
  input.maxLength = 60;
  input.value = member.displayName || pokemon.displayName;
  input.placeholder = canonicalDisplayName;
  ...
  form.addEventListener('submit', event => {
    ...
    if (setTeamPokemonAlias(team.id, member.id, input.value, canonicalDisplayName)) render();
  });
...
function createMemberCard(team, member, index, render) {
  ...
  edit.setAttribute('aria-label', `Edit name for ${pokemon.displayName}`);
  ...
  handle.setAttribute('aria-label', `Drag to reorder ${pokemon.displayName}`);
  ...
  details.addEventListener('click', () => {
    expanded ? expandedMembers.delete(expansionKey) : expandedMembers.add(expansionKey);
    render();
  });
  ...
  function updateInsertionMarker(clientY) {
    const roster = card.closest('.team-detail-roster');
    if (!roster) return;
    ...
    if (insertBefore) roster.insertBefore(insertionMarker, insertBefore);
    else roster.append(insertionMarker);
  }
  ...
  function stopDrag() {
    ...
    if (proposedIndex !== index) reorderPokemonInTeam(team.id, index, proposedIndex);
    render();
  }
...
function createRoster(team, render) {
  ...
  if (!team.pokemon.length) {
    ...
    team.pokemon.forEach((member, index) => roster.append(createMemberCard(team, member, index, render)));
  }
  return roster;
}
...
function getAnalysisCaption(mode, relationships) {
  ...
  if (mode === 'defense') return relationship === 'weak' ? 'Incoming move types that are super effective against each Pokémon.' : 'Incoming move types that each Pokémon resists or is immune to.';
...
function getMatchupResult(member, type, mode, relationship) {
  if (mode === 'defense') {
    ...
    return { marked: relationship === 'weak' ? multiplier > 1 : multiplier < 1, label: `${type} moves deal ${multiplier}× damage to ${member.displayName}` };
  }
  ...
  if (relationship === 'strong') {
    ...
    return { marked: effective.length > 0, label: `${member.displayName}: ${effective.map(entry => entry.attackingType).join(' or ')} moves are super effective against ${type}` };
  }
...
function getResolvedTeamPokemon(team) {
  return team.pokemon
    .map(member => {
      const resolved = resolvedPokemon.get(member.id);
      return resolved ? { ...resolved, displayName: member.displayName || resolved.displayName } : null;
    })
...
function createAnalysisTable(pokemon, mode, relationships, team) {
  ...
  for (const type of TYPES) {
    ...
    for (const member of pokemon) {
      const results = [...relationships].map(relationship => ({ ...getMatchupResult(member, type, mode, relationship), role: getRelationshipRole(mode, relationship, team.isOpponent === true) }));
...
function createAnalysisSelector(className, label, options, activeValue, onChange) {
  ...
  for (const option of options) {
    ...
    const button = el('button', { className: active ? 'primary-button' : 'secondary-button', text: option.label });
    button.type = 'button';
    button.setAttribute('role', 'tab');
...
function createRelationshipToggles(mode, team, render) {
  ...
  for (const option of options) {
    ...
    const button = el('button', { className: `secondary-button team-analysis-status-${role}${active ? ' team-analysis-status-active' : ''}`, text: option.label });
    button.type = 'button';
...
function createAnalysis(team, render) {
  ...
  section.append(createAnalysisSelector('team-analysis-tabs', 'Team matchup direction', [{ value: 'defense', label: 'Defense' }, { value: 'offense', label: 'Offense' }], activeAnalysisMode, mode => {
  ...
  if (pokemon.length !== team.pokemon.length) section.append(el('p', { className: 'muted', text: 'Loading matchup data…' }));
  else if (!pokemon.length) section.append(el('p', { className: 'muted', text: 'Add Pokémon to this team to analyze its matchups.' }));
  else section.append(createAnalysisTable(pokemon, activeAnalysisMode, relationships, team));
  return section;
}
...
function createOverallMatchupMatrix(team) {
  ...
  const caption = document.createElement('caption');
  caption.textContent = isOpponent ? 'Types that have an overall advantage against each opponent Pokémon.' : 'Types that each Pokémon has an overall advantage against.';
  table.append(caption);
  const head = document.createElement('thead');
  const headRow = document.createElement('tr');
  const corner = document.createElement('th');
  ...
  for (const type of TYPES) {
    ...
    for (const member of pokemon) {
      ...
      if (shouldMark) {
        ...
        const dot = el('span', { className: `team-advantage-dot team-advantage-dot-${role} team-advantage-dot-${intensity}` });
        const perspectiveScore = isOpponent ? -score : score;
        const label = isOpponent ? `${type} has advantage score ${perspectiveScore} against ${member.displayName}` : `${member.displayName} has advantage score ${score} against ${type}`;
...
export function renderTeamDetail(container, render) {
  ...
  heading.append(el('h2', { className: `team-detail-title${team.isOpponent ? ' team-detail-title-opponent' : ''}`, text: team.title }), createOpponentToggle(team, render));
  ...
  else page.append(createOverallMatchupMatrix(team));
  container.replaceChildren(page);
